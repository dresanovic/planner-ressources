...
- Q: Which Lecturer, Cohort, and Room choices should manual session creation allow? → A: Default all three from the Course, but offer all active Lecturers and Cohorts plus capacity-valid active Rooms as simple dropdown choices; save overrides only on the new Draft Session without changing the Course.
...
5. **Given** the selected course has inherited Lecturer, Cohort, and Room values, **When** the planner prepares a manual session, **Then** those values are selected by default and the planner may override them from dropdowns containing all active Lecturers, all active Cohorts, and active Rooms whose capacity is sufficient for the currently selected Cohort, without changing the Course.
...
- **FR-003**: A manual Draft Session MUST identify one existing course-semester context, one date, one start time, one end time, a positive whole number of teaching units, one existing active Lecturer, one existing active Cohort, and one existing active Room selected by the planner.
...
- **FR-006**: The system MUST reject a manual session when any required course, semester, planner-selected Lecturer, Cohort, or Room reference does not exist or is inactive.
...
- **FR-008**: The system MUST reject a manual session whose assigned room capacity is below the planner-selected Cohort size.
...
- **Draft Session**: One generated or manually created teaching occurrence with a date, start and end time, explicit teaching-unit count, one Lecturer, one Cohort, and one Room; manual creation defaults these values from the Course while allowing session-only overrides.
...
- A manual Draft Session initially inherits the course's existing Lecturer, Cohort, and preferred Room. The planner may override them with any active Lecturer, any active Cohort, and any active Room that passes the selected Cohort's hard capacity rule. These overrides affect only the new session; this slice does not require course eligibility for manual lecturer selection and does not modify the Course.
