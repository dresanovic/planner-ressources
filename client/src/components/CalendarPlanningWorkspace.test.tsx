...
describe('CalendarPlanningWorkspace', () => {
  it('shows revision identity, five truthful summaries, and teaching/exam distinction', () => {
    ...
    expect(document.querySelector<HTMLElement>('[aria-label="List mode"]')?.hidden).toBe(true)
  ...
  it('uses the established Courses overview as its only List mode', () => {
    ...
    expect(button('Current period not applicable')?.disabled).toBe(true)
  })
  ...
  it('keeps the existing List workflow mounted when another calendar mode is shown', () => {
    ...
    act(() => modeButton('Week').click())
    expect(host.querySelector('[aria-label="List mode"]')?.hasAttribute('hidden')).toBe(true)
    act(() => modeButton('List').click())
  ...
  it('passes the unified filtered projection and exact undated trace target to List', () => {
    ...
    expect(host.textContent).toContain('Filtered subset')
  })
  ...
  it('reconciles open drilldown contributors when filters change', () => {
    ...
    const courseFilter = [...host.querySelectorAll<HTMLSelectElement>('select')]
      .find((item) => item.previousElementSibling?.textContent === 'Course')!
    act(() => {
    ...
    expect(host.querySelector('.workspace-drilldown')?.textContent).toContain(
      '1 affected record in the filtered subset',
    )
  ...
  it('uses a predictable focus fallback when filtering disables the drilldown initiator', async () => {
    ...
    act(() => [...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent?.trim() === 'Clear drilldown')!.click())
    ...
    act(() => [...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent?.trim() === 'Clear filters')!.click())
    act(() => needsReview.click())
    selectCleanCourse()
    act(() => [...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent?.trim() === 'Clear filters')!.click())
  ...
  it('restores calendar context and focus through either drilldown clear path', async () => {
    ...
    const capacityIssues = [...host.querySelectorAll<HTMLButtonElement>('.summary-card')]
      .find((item) => item.textContent?.includes('Capacity issues'))!
    ...
    const openDatedContributor = () => {
      ...
      const contributor = [...host.querySelectorAll<HTMLButtonElement>('.workspace-drilldown button')]
        .find((item) => item.textContent?.includes('2026-12-10'))!
      act(() => contributor.click())
    }

    expect(inHostButton('Week').getAttribute('aria-pressed')).toBe('true')
    openDatedContributor()

    expect(inHostButton('Day').getAttribute('aria-pressed')).toBe('true')
    expect(host.querySelector('.visible-range')?.textContent).toBe('2026-12-10')
    expect(host.textContent).toContain('Exam session')
    expect(host.textContent).toContain('Revision 1')
    expect(host.textContent).toContain('Active Working')

    act(() => inHostButton('Clear filters').click())
    await act(async () => {
      await new Promise((resolve) => window.setTimeout(resolve, 0))
    })
    expect(inHostButton('Week').getAttribute('aria-pressed')).toBe('true')
    expect(host.querySelector('.visible-range')?.textContent).toContain('2026-10-05')
    ...
    openDatedContributor()
    act(() => inHostButton('Clear drilldown').click())
    await act(async () => {
      await new Promise((resolve) => window.setTimeout(resolve, 0))
    })
    expect(inHostButton('Week').getAttribute('aria-pressed')).toBe('true')
  ...
  it('routes undated outcomes and findings to exact List targets with related conflict linkage', () => {
    ...
    act(() => summary('Planning failures').click())
    act(() => drilldownContributor('semester optimization').click())
    expect(contexts.at(-1)).toMatchObject({
      courseIds: [1],
      teachingSessionIds: [1],
      examIds: [1],
    ...
    const clear = [...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent?.trim() === 'Clear drilldown')!
    act(() => clear.click())
    act(() => summary('Conflicts').click())
    act(() => drilldownContributor('room conflict').click())
    expect(contexts.at(-1)).toMatchObject({
      traceTarget: {
  ...
  it('navigates Day, Week, Month, and List while preserving active filters', () => {
    ...
    act(() => inHostButton('Day').click())
    expect(host.querySelector('.visible-range')?.textContent).toBe('2026-10-05')
    act(() => host.querySelector<HTMLButtonElement>('[aria-label="Previous period"]')!.click())
    expect(host.querySelector('.visible-range')?.textContent).toBe('2026-10-04')
    act(() => inHostButton('Month').click())
    expect(host.querySelector('.visible-range')?.textContent).toContain('2026-10-01')
    act(() => inHostButton('List').click())
    expect(sessionType.value).toBe('teaching')
    expect(inHostButton('Current period not applicable').disabled).toBe(true)
    expect(host.querySelector<HTMLElement>('[aria-label="List mode"]')?.hidden).toBe(false)
  })
  ...
  it('excludes same-course findings whose occurrences are outside a resource filter', () => {
    ...
    const roomFilter = [...host.querySelectorAll<HTMLSelectElement>('select')]
      .find((item) => item.previousElementSibling?.textContent === 'Room')!
    act(() => {
    ...
    const conflict = [...host.querySelectorAll<HTMLButtonElement>('.summary-card')]
      .find((item) => item.textContent?.includes('Conflicts'))!
    expect(conflict.querySelector('strong')?.textContent).toBe('0')
    expect(conflict.textContent).toContain('Filtered subset')
  })
  ...
  it('hands a teaching edit to the established List editor', () => {
    ...
    const edit = [...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent?.trim() === 'Edit with existing editor')
    act(() => edit?.click())
  ...
  it('shows complete exam capacity, lifecycle, recommendation, and validity detail', () => {
    ...
    const exam = [...host.querySelectorAll<HTMLButtonElement>('.calendar-occurrence')]
      .find((item) => item.textContent?.includes('Exam'))!
    act(() => exam.click())

    expect(host.textContent).toContain('30 required')
    expect(host.textContent).toContain('Auditorium 1')
    expect(host.textContent).toContain('40 current')
    expect(host.textContent).toContain('Draft')
    expect(host.textContent).toContain('FINAL')
    expect(host.textContent).toContain('planner override')
  })
  ...
  it('exposes a count and keyboard-operable continuation for a dense Month date', () => {
    ...
    const month = [...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent?.trim() === 'Month')!
    act(() => month.click())

    expect(host.textContent).toContain('6 sessions')
    ...
    const showAll = [...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent?.includes('Show all 6 sessions'))!
    act(() => showAll.click())
    expect(host.querySelectorAll('.calendar-occurrence')).toHaveLength(6)
    expect(host.textContent).toContain('Show fewer sessions')
  })
  ...
  it('moves focus and explains when the selected result stops matching filters', async () => {
    ...
    expect(host.textContent).toContain('The selected session no longer matches the current result set.')
  })
  ...
  it('announces successful recovery after a failed workspace load', async () => {
    ...
    expect(host.querySelector('[data-workspace-recovery-announcement]')?.textContent)
      .toContain('Calendar workspace recovered')
  })
  ...
  it('shows a no-revision action and a Published read-only explanation', () => {
    ...
    expect(noRevision.textContent).toContain('Start Draft')
  ...
  it('recomputes partial metric coverage within verified, unverified, and mixed filters', () => {
    ...
    const studyTypeFilter = [...host.querySelectorAll<HTMLSelectElement>('select')]
      .find((item) => item.previousElementSibling?.textContent === 'Study type')!
    const summary = (label: string) => [...host.querySelectorAll<HTMLButtonElement>('.summary-card')]
      .find((item) => item.textContent?.includes(label))!
    ...
    expect(summary('Capacity issues').querySelector('strong')?.textContent).toBe('0')
    expect(summary('Capacity issues').textContent).not.toContain('Known incomplete')
    expect(summary('Needs review').querySelector('strong')?.textContent).toBe('1')
    expect(summary('Needs review').textContent).not.toContain('Known incomplete')

    select(courseFilter, 'course:2')
    expect(summary('Capacity issues').querySelector('strong')?.textContent).toBe('Unavailable')
    expect(summary('Needs review').querySelector('strong')?.textContent).toBe('Unavailable')

    select(courseFilter, '')
    select(studyTypeFilter, 'Full-time')
    expect(summary('Capacity issues').textContent).toContain('Known incomplete')
    expect(summary('Needs review').textContent).toContain('Known incomplete')
  })
  ...
  it('uses only planning-eligible courses for filtered outcome coverage', () => {
    ...
    const planningFailures = () => [...host.querySelectorAll<HTMLButtonElement>('.summary-card')]
      .find((item) => item.textContent?.includes('Planning failures'))!
    ...
    expect(planningFailures().textContent).toContain('0 failed')
    expect(planningFailures().textContent).not.toContain('Known incomplete')

    selectCourse('course:2')
    expect(planningFailures().querySelector('strong')?.textContent).toBe('Not applicable')
  })
  ...
  it('identifies partial sections and initial failure without false current values', () => {
    ...
    expect(partial.textContent).toContain('Some workspace information is incomplete.')
    expect(partial.textContent).toContain('Current resource data could not be loaded.')
    act(() => partial.remove())
    ...
    act(() => root.render(
      <CalendarPlanningWorkspace
        ...
        loading={false}
        error="Network unavailable."
        intendedContext="Fall 2026 · Revision 2 · Current Published"
        listContent={<div>Existing Courses overview</div>}
        onRetry={vi.fn()}
      />,
    ))

    expect(host.textContent).toContain('Fall 2026 · Revision 2 · Current Published')
  ...
  it('distinguishes loading, no-semester, no-session, no-match, and last-known refresh states', () => {
    ...
    expect(host.textContent).toContain('Loading semester workspace')
    act(() => root.render(<CalendarPlanningWorkspace {...common} workspace={null} loading={false} />))
    expect(host.textContent).toContain('No semester selected')
    ...
    expect(host.textContent).toContain('No sessions occur in this period.')
    ...
    expect([...host.querySelectorAll<HTMLButtonElement>('button')]
      .find((item) => item.textContent === 'Clear filters')?.disabled).toBe(false)
