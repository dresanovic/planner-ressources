...
it('always shows semester and only destination-meaningful revision/course controls', () => {
  ...
  expect([...host.querySelectorAll('label span')].map((item) => item.textContent)).toEqual(['Semester', 'Revision', 'Lehrveranstaltung'])
  ...
  expect([...host.querySelectorAll('label span')].map((item) => item.textContent)).toEqual(['Semester', 'Lehrveranstaltung'])
  act(() => root.render(<ScheduleContextHeader destination="reviews" {...props} />))
  expect(host.querySelector('h2')?.textContent).toBe('Abstimmung mit Lehrende')
  expect([...host.querySelectorAll('label span')].map((item) => item.textContent)).toEqual(['Semester', 'Revision'])
})
...
it('makes unavailable context explicit without color-only meaning', () => {
  ...
  expect(host.textContent).toContain('Keine Revision verfügbar')
  expect(host.textContent).toContain('Keine Lehrveranstaltung verfügbar')
})
