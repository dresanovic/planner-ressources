...
describe('conflict-aware generation API', () => {
  it('prepares 1-20 courses with future unavailable dates', async () => {
    ...
    const result = await prepareConflictAwareGeneration(1, [1, 2], ['2026-10-26'])
    expect(result.sharedSnapshotToken).toBe('shared-snapshot')
    expect(JSON.parse(String(fetchMock.mock.calls[0][1]?.body))).toEqual({ semesterId: 1, courseIds: [1, 2], unavailableDates: ['2026-10-26'] })
  })
  ...
  it('parses validation and solver failures and normalizes network failure', async () => {
    ...
    await expect(prepareConflictAwareGeneration(1, [1], [])).rejects.toEqual([{ code: 'NETWORK_ERROR', message: 'Could not reach the backend API.' }])
  })
})
