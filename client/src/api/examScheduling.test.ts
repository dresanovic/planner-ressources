...
describe('exam scheduling API', () => {
  it('serializes overview, configuration, preparation, generation, and manual mutations exactly', async () => {
    const responses = [
      ...
      { courseId: 1 },
      { semesterId: 1, scheduleRevisionId: 11, institutionToday: '2026-09-01', sharedSnapshotToken: 'shared', courses: [] },
    ...
    await prepareExamGeneration(1, 11, [1])
    await generateExams({ semesterId: 1, scheduleRevisionId: 11, institutionToday: '2026-09-01', sharedSnapshotToken: 'shared', courses: [] })
    await createManualExam(1, { semesterId: 1, scheduleRevisionId: 11, date: '2026-10-16', startTime: '09:00', lecturerId: 1, roomId: 1, expectedConfigurationRevision: 1, inputSnapshotToken: 'c' })
    await updateExam(2, { scheduleRevisionId: 11, date: '2026-10-17', startTime: '10:00', lecturerId: 1, roomId: 1, expectedExamRevision: 1, inputSnapshotToken: 'e' })
    await deleteExam(2, { scheduleRevisionId: 11, confirmed: true, expectedExamRevision: 2, inputSnapshotToken: 'e2' })
  ...
  it('validates 1-100 unique selections and retains structured stale state', async () => {
    await expect(prepareExamGeneration(1, 11, [])).rejects.toMatchObject({ errors: [{ code: 'INVALID_SELECTION' }] })
    await expect(prepareExamGeneration(1, 11, [1, 1])).rejects.toMatchObject({ errors: [{ code: 'INVALID_SELECTION' }] })
